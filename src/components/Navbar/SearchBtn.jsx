...
const SearchBtn = () => {
    ...
    const [openSearch, setOpenSeacrh] = React.useState(false)
    const [chosenSearch, setChosenSearch] = React.useState(0)

    const onClickSelect =()=>{
        setOpenSeacrh(prevState => !prevState)
    }
        ...
        <div className="select_button" onClick={onClickSelect} >
            ...
            <ul style={{display: openSearch? "flex": "none"}}>
                {searchNames.map((item, index)=>(
                    <li onClick={()=>{onClickChosen(index)}} className= {chosenSearch === index && "chosenSearch"}>{item} </li>
                ))}
            </ul>
        </div>
